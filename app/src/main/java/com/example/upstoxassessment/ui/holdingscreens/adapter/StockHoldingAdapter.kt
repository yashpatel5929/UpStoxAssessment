...
class StockHoldingAdapter : RecyclerView.Adapter<StockHoldingAdapter.ViewHolder>() {
    ...
    inner class ViewHolder(val binding : ItemListStockHoldingBinding) : RecyclerView.ViewHolder(binding.root) {
        fun bind(item: StockHoldingData) {
            binding.tvSymbol.text = item.symbol
            binding.tvNetQuantitValue.text = item.quantity?.toString()
            binding.tvLtpValue.text = itemView.context.getString(R.string.ltp_value_string , item.ltp)
        }
    }
    override fun onCreateViewHolder(
       ...
       val view = ItemListStockHoldingBinding.inflate(LayoutInflater.from(parent.context),parent,false)
